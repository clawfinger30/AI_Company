...
updated_at: 2026-07-20T12:37:42+09:00
...
2026-07-20にユーザーが推奨設定での実装を承認した。

1. 月額500円、自動更新、無料体験なしで初期公開する。
2. 商品IDは`premium_monthly_android`、基本プランIDは`monthly`とする。
3. v0.7はクライアントで購入状態を確認し、サーバー検証・RTDNはv2.0へ移す。
4. 通信失敗時は、直前に確認済みの有効権利を最終確認から3日間保持する。
5. Play Consoleの猶予期間は7日、アカウント一時停止期間は自動設定とする。
